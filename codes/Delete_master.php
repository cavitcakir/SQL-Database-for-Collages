...
color: white;
	...
	<h3 class="text-white text-center">Delete People from ID</h3>
		<form action="Delete_master.php" method="post">
      <div class="text-center">
			<div class="form-group">
			    <input type="text" name="ID" placeholder="Enter ID">
			</div>
			<button type="submit" name="search" class="btn btn-success" align="middle">Delete</button>
      </div>
		</form>
...
{
	$del_ID = $_POST['ID'];
	$query = "SELECT ID FROM People  WHERE ID = '$del_ID' ";
    ...
    if($row_cnt == 1)
    {
      $query = "SELECT ID FROM student WHERE ID = '$del_ID' ";
      $response = mysqli_query($link, $query);
      $row_cnt = mysqli_num_rows($response);

      $query = "DELETE FROM People WHERE ID = '$del_ID' ";
      mysqli_query($link, $query); //Delete from people table

      if($row_cnt == 1)  //Person is student
      {
       // $query = "DELETE FROM STUDENT WHERE ID = '$del_ID' ";
       // mysqli_query($link, $query); //Delete from student table
        echo "<script>alert('Student is deleted successfully');</script>";
      }
      else
      {
        //$query = "DELETE FROM INSTRUCTOR WHERE ID = '$del_ID' ";
        //mysqli_query($link, $query); //Delete from Instructor table
        echo "<script>alert('Instructor is deleted successfully');</script>";
      }
    }
    else
    {
      echo "<script>alert('Could not find user that you are searching for. Please try again.');</script>";
    }
	}
	else
	{
		echo "<script>alert('Couldn't issue database query. Something went wrong.');</script>";
		header("Refresh:0; url=Delete_master.php");
	}
}
