...
<html>
  ...
  $check = mysqli_query($link,"SELECT CT.Course_Code, CT.Name AS Course_Name, P.Name AS Course_Teacher, CT.Credit, CT.Class, CT.Time 
  FROM belongs_to B, course_teacher CT , People P 
  WHERE B.Course_Code = CT.Course_Code  AND P.ID = CT.Instructor_Id");
...
<table class="table table-striped table-hover table-responsive" style= "background-color : white ">
        ...
        while($row = mysqli_fetch_array($records))
          ...
          echo "<tr><form action=Students_table_update2_master.php method = post>";
          echo "<td><input type = text name = ID value='".$row['ID']."'</td>";
          echo "<td><input type = text name = Major value='".$row['Major']."'</td>";
          echo "<td><input type = text name = Entrance_Year value='".$row['Entrance_Year']."'</td>";
          echo "<td><input type = text name = Total_Credit value='".$row['Total_Credit']."'</td>";
